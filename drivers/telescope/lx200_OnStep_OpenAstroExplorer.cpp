/*
    LX200 LX200_OnStep_OpenAstroExplorer
...
#define POLAR_ADJUSTMENT_TAB "Polar Adjustment"
...
LX200_OnStep_OpenAstroExplorer::LX200_OnStep_OpenAstroExplorer() : LX200Generic(), WI(this), RotatorInterface(this)
{
    currentCatalog    = LX200_STAR_C;
    currentSubCatalog = 0;

    setVersion(1, 0);   // don't forget to update libindi/drivers.xml
...
const char *LX200_OnStep_OpenAstroExplorer::getDefaultName()
{
    return "LX200 OnStep OpenAstroExplorer";
}

bool LX200_OnStep_OpenAstroExplorer::initProperties()
{
    ...
    // ============== POLAR_ADJUSTMENT_TAB
    OAEAltitudeJogNP[0].fill("ALTITUDE_OFFSET", "Altitude correction (D:M:S)", "%10.6m",
                             -3.0, 3.0, 1.0 / 3600.0, 0.0);
    OAEAltitudeJogNP.fill(getDeviceName(), "OAE_ALTITUDE_JOG", "Polar altitude correction",
                          POLAR_ADJUSTMENT_TAB, IP_RW, 0, IPS_IDLE);

    OAEAltitudeJogSP[0].fill("JOG", "Jog altitude", ISS_OFF);
    OAEAltitudeJogSP.fill(getDeviceName(), "OAE_ALTITUDE_JOG_COMMAND", "Apply altitude correction",
                          POLAR_ADJUSTMENT_TAB, IP_RW, ISR_ATMOST1, 0, IPS_IDLE);

    OAEAzimuthJogNP[0].fill("AZIMUTH_OFFSET", "Azimuth correction (D:M:S)", "%10.6m",
                            -3.0, 3.0, 1.0 / 3600.0, 0.0);
    OAEAzimuthJogNP.fill(getDeviceName(), "OAE_AZIMUTH_JOG", "Polar azimuth correction",
                         POLAR_ADJUSTMENT_TAB, IP_RW, 0, IPS_IDLE);

    OAEAzimuthJogSP[0].fill("JOG", "Jog azimuth", ISS_OFF);
    OAEAzimuthJogSP.fill(getDeviceName(), "OAE_AZIMUTH_JOG_COMMAND", "Apply azimuth correction",
                         POLAR_ADJUSTMENT_TAB, IP_RW, ISR_ATMOST1, 0, IPS_IDLE);
...
void LX200_OnStep_OpenAstroExplorer::ISGetProperties(const char *dev)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) != 0) return;
    LX200Generic::ISGetProperties(dev);
}

bool LX200_OnStep_OpenAstroExplorer::updateProperties()
{
    ...
    if (isConnected())
    {
        ...
        resetPolarAdjustmentState();
        if (isSimulation())
            setSimulatedPolarScales();
        else
            queryPolarAdjustmentAxes();

        defineProperty(OAEAltitudeJogNP);
        defineProperty(OAEAltitudeJogSP);
        defineProperty(OAEAzimuthJogNP);
        defineProperty(OAEAzimuthJogSP);
        ...
        else     //For OnStepX, up to 6 focusers
        {
            ...
            OSFocuser2 = false;
            if (OnStepMountVersion == OAE_OSV_UNKNOWN || OnStepMountVersion == OAE_OSV_OnStepX)
            {
    ...
    else
    {
        ...
        deleteProperty(OAEAltitudeJogNP);
        deleteProperty(OAEAltitudeJogSP);
        deleteProperty(OAEAzimuthJogNP);
        deleteProperty(OAEAzimuthJogSP);
        resetPolarAdjustmentState();
...
bool LX200_OnStep_OpenAstroExplorer::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        if (OAEAltitudeJogNP.isNameMatch(name))
            return processPolarAdjustmentNumber(OAEAltitudeJogNP, "altitude", values, names, n);

        if (OAEAzimuthJogNP.isNameMatch(name))
            return processPolarAdjustmentNumber(OAEAzimuthJogNP, "azimuth", values, names, n);
...
bool LX200_OnStep_OpenAstroExplorer::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
{
    int index = 0;

    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        if (OAEAltitudeJogSP.isNameMatch(name))
        {
            OAEAltitudeJogSP.update(states, names, n);
            const bool jogRequested = OAEAltitudeJogSP[0].getState() == ISS_ON;
            OAEAltitudeJogSP.reset();
            if (!jogRequested)
            {
                OAEAltitudeJogSP.setState(IPS_IDLE);
                OAEAltitudeJogSP.apply();
                return true;
            }
            return processPolarJog(OAEAltitudeJogSP, "altitude", 1, OAEAltitudeJogNP,
                                   OAEAltitudeAxisAvailable, OAEAltitudeArcsecondsPerRawStep);
        }

        if (OAEAzimuthJogSP.isNameMatch(name))
        {
            OAEAzimuthJogSP.update(states, names, n);
            const bool jogRequested = OAEAzimuthJogSP[0].getState() == ISS_ON;
            OAEAzimuthJogSP.reset();
            if (!jogRequested)
            {
                OAEAzimuthJogSP.setState(IPS_IDLE);
                OAEAzimuthJogSP.apply();
                return true;
            }
            return processPolarJog(OAEAzimuthJogSP, "azimuth", 2, OAEAzimuthJogNP,
                                   OAEAzimuthAxisAvailable, OAEAzimuthArcsecondsPerRawStep);
        }
...
void LX200_OnStep_OpenAstroExplorer::getBasicData()
{
    // process parent
    ...
    if (!isSimulation())
    {
                ...
                && (strcmp(VersionTP[3].getText(), "OnStep") || strcmp(VersionTP[3].getText(), "On-Step")))
        {
            ...
            OnStepMountVersion = OAE_OSV_OnStepV1or2;
        }
                 ...
                 || strcmp(VersionTP[3].getText(), "On-Step")))
        {
            ...
            OSHighPrecision = false;
            OnStepMountVersion = OAE_OSV_OnStepV3;
        }
                 ...
                 || strcmp(VersionTP[3].getText(), "On-Step")))
        {
            ...
            OnStepMountVersion = OAE_OSV_OnStepV4;
        }
                 ...
                 || strcmp(VersionTP[3].getText(), "On-Step")))
        {
            ...
            OSHighPrecision = true;
            OnStepMountVersion = OAE_OSV_OnStepV5;
        }
                 ...
                 && (strcmp(VersionTP[3].getText(), "OnStepX") || strcmp(VersionTP[3].getText(), "On-Step")))
        {
            ...
            OSHighPrecision = true;
            OnStepMountVersion = OAE_OSV_OnStepX;
        }
        else
        {
            LOG_INFO("OnStep/OnStepX version could not be detected");
            OSHighPrecision = false;
            OnStepMountVersion = OAE_OSV_UNKNOWN;
        }
...
//======================== Parking =======================
bool LX200_OnStep_OpenAstroExplorer::SetCurrentPark()
{
    char response[RB_MAX_LEN];
    // 0 = failure, 1 = success
...
bool LX200_OnStep_OpenAstroExplorer::SetDefaultPark()
{
...
bool LX200_OnStep_OpenAstroExplorer::UnPark()
{
    char response[RB_MAX_LEN];
...
bool LX200_OnStep_OpenAstroExplorer::Park()
{
    if (!isSimulation())
    {
...
bool LX200_OnStep_OpenAstroExplorer::ReadScopeStatus()
{
    ...
    OAE_Errors Lasterror = OAE_ERR_NONE;
    ...
    if (!OSSupports_bitfield_Gu)
    {
        ...
        if (error_or_fail > 1) // check if successful read (strcmp(OSStat, OldOSStat) != 0) //if status changed
        {
            ...
            if (OSPECviaGU)
            {
                if (OSMountType != OAE_MOUNTTYPE_ALTAZ && OSMountType != OAE_MOUNTTYPE_FORK_ALT )
                {
                    ...
                    if ((capabilities | TELESCOPE_HAS_PEC) != capabilities)
                    {
                        ...
                        initSlewRates();
                        LX200_OnStep_OpenAstroExplorer::updateProperties();
                    }
                }
                OSPECStatusSP.apply();
                OSPECRecordSP.apply();
            ...
            // ============= Error Code
            //From OnStep: OAE_ERR_NONE, OAE_ERR_MOTOR_FAULT, OAE_ERR_ALT_MIN, OAE_ERR_LIMIT_SENSE, OAE_ERR_DEC, OAE_ERR_AZM, OAE_ERR_UNDER_POLE, OAE_ERR_MERIDIAN, OAE_ERR_SYNC, OAE_ERR_PARK, OAE_ERR_GOTO_SYNC, OAE_ERR_UNSPECIFIED, OAE_ERR_ALT_MAX, OAE_ERR_GOTO_ERR_NONE, OAE_ERR_GOTO_ERR_BELOW_HORIZON, OAE_ERR_GOTO_ERR_ABOVE_OVERHEAD, OAE_ERR_GOTO_ERR_STANDBY, OAE_ERR_GOTO_ERR_PARK, OAE_ERR_GOTO_ERR_GOTO, OAE_ERR_GOTO_ERR_OUTSIDE_LIMITS, OAE_ERR_GOTO_ERR_HARDWARE_FAULT, OAE_ERR_GOTO_ERR_IN_MOTION, OAE_ERR_GOTO_ERR_UNSPECIFIED
            ...
            Lasterror = (OAE_Errors)(OSStat[strlen(OSStat) - 1] - '0');
    ...
    else
    {
        ...
        int pulseguiderateint = (OAE_Errors)(OSStat[strlen(OSStat) - 3] - '0');
...
#ifndef OnStep_Alpha
    if (OSMountType == OAE_MOUNTTYPE_GEM)
    {
        //AutoFlip
    ...
    if (OSMountType == OAE_MOUNTTYPE_GEM)   //Doesn't apply to non-GEMs
    {
        //PreferredPierSide
...
bool LX200_OnStep_OpenAstroExplorer::SetTrackEnabled(bool enabled) //track On/Off events handled by inditelescope       Tested
{
    char response[RB_MAX_LEN];
...
bool LX200_OnStep_OpenAstroExplorer::setLocalDate(uint8_t days, uint8_t months, uint16_t years)
{
    years = years % 100;
    char cmd[CMD_MAX_LEN] = {0};
...
bool LX200_OnStep_OpenAstroExplorer::sendOnStepCommandBlind(const char *cmd)
{
    int error_type;
    int nbytes_write = 0;
...
bool LX200_OnStep_OpenAstroExplorer::sendOnStepCommand(const char *cmd)
{
    char response[1] = {0};
    int error_type;
...
int LX200_OnStep_OpenAstroExplorer::getCommandSingleCharResponse(int fd, char *data, const char *cmd)
{
    char *term;
    int error_type;
    int nbytes_write = 0, nbytes_read = 0;
...
int LX200_OnStep_OpenAstroExplorer::flushIO(int fd)
{
...
int LX200_OnStep_OpenAstroExplorer::getCommandDoubleResponse(int fd, double *value, char *data, const char *cmd)
{
    char *term;
    int error_type;
    int nbytes_write = 0, nbytes_read = 0;
    ...
    if (sscanf(data, "%lf", value) != 1)
    {
        ...
        LOG_DEBUG("Flushing connection");
        tcflush(fd, TCIOFLUSH);
        return OAE_RES_ERR_FORMAT; //-1001, so as not to conflict with TTY_RESPONSE;
    }
...
int LX200_OnStep_OpenAstroExplorer::getCommandIntResponse(int fd, int *value, char *data, const char *cmd)
{
    char *term;
    int error_type;
    int nbytes_write = 0, nbytes_read = 0;
...
int LX200_OnStep_OpenAstroExplorer::getCommandSingleCharErrorOrLongResponse(int fd, char *data, const char *cmd)
{
    char *term;
    int error_type;
    int nbytes_write = 0, nbytes_read = 0;
...
bool LX200_OnStep_OpenAstroExplorer::updateLocation(double latitude, double longitude, double elevation)
{
...
int LX200_OnStep_OpenAstroExplorer::setMinElevationLimit(int fd, int max)   // According to standard command is :SoDD*#       Tested
{
...
int LX200_OnStep_OpenAstroExplorer::setSiteLongitude(int fd, double Long)
{
    int d, m;
    double s;
    char read_buffer[32];
...
int LX200_OnStep_OpenAstroExplorer::setSiteLatitude(int fd, double Long)
{
    int d, m;
    double s;
    char read_buffer[32];
...
IPState LX200_OnStep_OpenAstroExplorer::MoveFocuser(FocusDirection dir, int speed, uint16_t duration)
{
    INDI_UNUSED(speed);
...
IPState LX200_OnStep_OpenAstroExplorer::MoveAbsFocuser (uint32_t targetTicks)
{
...
IPState LX200_OnStep_OpenAstroExplorer::MoveRelFocuser (FocusDirection dir, uint32_t ticks)
{
    //  :FRsnnn#  Set focuser target position relative (in microns)
    //            Returns: Nothing
    int output;
...
bool LX200_OnStep_OpenAstroExplorer::AbortFocuser ()
{
    //  :FQ#   Stop the focuser
    //         Returns: Nothing
    char cmd[CMD_MAX_LEN] = {0};
...
int LX200_OnStep_OpenAstroExplorer::OSUpdateFocuser()
{
...
int LX200_OnStep_OpenAstroExplorer::OSUpdateRotator()
{
    ...
    if(OSRotator1)
    {
        ...
        if (OnStepMountVersion != OAE_OSV_OnStepV1or2 && OnStepMountVersion != OAE_OSV_OnStepV3)
        {
...
IPState LX200_OnStep_OpenAstroExplorer::MoveRotator(double angle)
{
    char cmd[CMD_MAX_LEN] = {0};
    int d, m, s;
...
/*
bool LX200_OnStep_OpenAstroExplorer::SyncRotator(double angle) {

}*/
IPState LX200_OnStep_OpenAstroExplorer::HomeRotator()
{
...
// bool LX200_OnStep_OpenAstroExplorer::ReverseRotator(bool enabled) {
//     sendOnStepCommandBlind(":rR#");
//     return true;
// } //No way to check which way it's going as Indi expects

bool LX200_OnStep_OpenAstroExplorer::AbortRotator()
{
...
bool LX200_OnStep_OpenAstroExplorer::SetRotatorBacklash(int32_t steps)
{
    char cmd[CMD_MAX_LEN] = {0};
    //     char response[RB_MAX_LEN];
...
bool LX200_OnStep_OpenAstroExplorer::SetRotatorBacklashEnabled(bool enabled)
{
...
IPState LX200_OnStep_OpenAstroExplorer::StartPECPlayback (int axis)
{
    //  :$QZ+  Enable RA PEC compensation
    //         Returns: nothing
    INDI_UNUSED(axis); //We only have RA on OnStep
    if (OSMountType != OAE_MOUNTTYPE_ALTAZ )
    {
...
IPState LX200_OnStep_OpenAstroExplorer::StopPECPlayback (int axis)
{
...
IPState LX200_OnStep_OpenAstroExplorer::StartPECRecord (int axis)
{
    //  :$QZ/  Ready Record PEC
    //         Returns: nothing
    INDI_UNUSED(axis); //We only have RA on OnStep
...
IPState LX200_OnStep_OpenAstroExplorer::ClearPECBuffer (int axis)
{
...
IPState LX200_OnStep_OpenAstroExplorer::SavePECBuffer (int axis)
{
...
IPState LX200_OnStep_OpenAstroExplorer::PECStatus (int axis)
{
    //     if (!OSPECviaGU) {
    INDI_UNUSED(axis); //We only have RA on OnStep
...
IPState LX200_OnStep_OpenAstroExplorer::ReadPECBuffer (int axis)
{
    INDI_UNUSED(axis); //We only have RA on OnStep
    if (OSPECEnabled == true)
    {
...
IPState LX200_OnStep_OpenAstroExplorer::WritePECBuffer (int axis)
{
    INDI_UNUSED(axis); //We only have RA on OnStep
    if (OSPECEnabled == true)
    {
...
IPState LX200_OnStep_OpenAstroExplorer::AlignStartGeometric (int stars)
{
...
IPState LX200_OnStep_OpenAstroExplorer::AlignAddStar ()
{
...
bool LX200_OnStep_OpenAstroExplorer::UpdateAlignStatus ()
{
    //  :A?#  Align status
    //         Returns: mno#
...
bool LX200_OnStep_OpenAstroExplorer::UpdateAlignErr()
{
...
IPState LX200_OnStep_OpenAstroExplorer::AlignDone()
{
    //See here https://groups.io/g/onstep/message/3624
...
IPState LX200_OnStep_OpenAstroExplorer::AlignWrite()
{
    //See here https://groups.io/g/onstep/message/3624
    char cmd[CMD_MAX_LEN] = {0};
    char response[RB_MAX_LEN];
...
#ifdef ONSTEP_NOTDONE
IPState LX200_OnStep_OpenAstroExplorer::OSEnableOutput(int output)
{
...
IPState LX200_OnStep_OpenAstroExplorer::OSDisableOutput(int output)
{
    LOG_INFO("Not implemented yet");
...
bool LX200_OnStep_OpenAstroExplorer::OSGetOutputState(int output)
{
    //  :GXnn#   Get OnStep value
    //         Returns: value
...
bool LX200_OnStep_OpenAstroExplorer::SetTrackRate(double raRate, double deRate)
{
...
void LX200_OnStep_OpenAstroExplorer::slewError(int slewCode)
{
    //         0=Goto is possible
    //         1=below the horizon limit
    //         2=above overhead limit
...
bool LX200_OnStep_OpenAstroExplorer::Sync(double ra, double dec)
{
...
void LX200_OnStep_OpenAstroExplorer::resetPolarAdjustmentState()
{
    OAEAltitudeAxisAvailable = false;
    OAEAzimuthAxisAvailable = false;
    OAEAltitudeArcsecondsPerRawStep = std::numeric_limits<double>::quiet_NaN();
    OAEAzimuthArcsecondsPerRawStep = std::numeric_limits<double>::quiet_NaN();
    OAEAltitudeJogSP.reset();
    OAEAzimuthJogSP.reset();
    OAEAltitudeJogSP.setState(IPS_IDLE);
    OAEAzimuthJogSP.setState(IPS_IDLE);
...
void LX200_OnStep_OpenAstroExplorer::setSimulatedPolarScales()
{
    OAEAltitudeAxisAvailable = true;
    OAEAzimuthAxisAvailable = true;
    OAEAltitudeArcsecondsPerRawStep = 0.25;
    OAEAzimuthArcsecondsPerRawStep = 0.25;
    OAEAltitudeJogSP.setState(IPS_OK);
    OAEAzimuthJogSP.setState(IPS_OK);
    LOG_DEBUG("OAE polar adjustment simulation scales: F1=0.25 arcsec/step, F2=0.25 arcsec/step");
}

void LX200_OnStep_OpenAstroExplorer::queryPolarAdjustmentAxes()
{
    OAEAltitudeAxisAvailable = queryPolarAxis(1, "F1 altitude", OAEAltitudeArcsecondsPerRawStep);
    OAEAltitudeJogSP.setState(OAEAltitudeAxisAvailable ? IPS_OK : IPS_ALERT);

    OAEAzimuthAxisAvailable = queryPolarAxis(2, "F2 azimuth", OAEAzimuthArcsecondsPerRawStep);
    OAEAzimuthJogSP.setState(OAEAzimuthAxisAvailable ? IPS_OK : IPS_ALERT);
}

bool LX200_OnStep_OpenAstroExplorer::queryPolarAxis(int axisNumber, const char *axisLabel, double &scale)
{
    if (!queryPolarAxisAvailability(axisNumber, axisLabel))
        return false;

    return queryPolarAxisScale(axisNumber, axisLabel, scale);
}
...
bool LX200_OnStep_OpenAstroExplorer::queryPolarAxisAvailability(int axisNumber, const char *axisLabel)
{
    ...
    char response[RB_MAX_LEN] = {0};
    const int result = getCommandSingleCharResponse(PortFD, response, cmd);
    if (result <= 0)
    {
        LOGF_ERROR("OAE %s axis availability query failed with result %d.", axisLabel, result);
        return false;
    }

    const bool available = response[0] == '1';
    LOGF_DEBUG("OAE %s axis availability: command=%s response=%c available=%s",
               axisLabel, cmd, response[0] ? response[0] : '?', available ? "true" : "false");
    if (!available)
        LOGF_ERROR("OAE %s axis is unavailable.", axisLabel);

    return available;
}

bool LX200_OnStep_OpenAstroExplorer::queryPolarAxisScale(int axisNumber, const char *axisLabel, double &scale)
{
    char cmd[CMD_MAX_LEN] = {0};
    const int written = snprintf(cmd, sizeof(cmd), ":F%du#", axisNumber);
    if (written < 0 || static_cast<size_t>(written) >= sizeof(cmd))
    {
        LOGF_ERROR("OAE %s scale command formatting failed.", axisLabel);
        return false;
    }

    char response[RB_MAX_LEN] = {0};
    const int result = getCommandSingleCharErrorOrLongResponse(PortFD, response, cmd);
    if (result <= 0)
    {
        LOGF_ERROR("OAE %s scale query failed with result %d.", axisLabel, result);
        return false;
    }

    double parsedScale = 0.0;
    if (!parsePositiveFiniteDouble(response, parsedScale))
    {
        LOGF_ERROR("OAE %s scale response is malformed or nonpositive: <%s>.", axisLabel, response);
        return false;
    }

    scale = parsedScale;
    LOGF_DEBUG("OAE %s scale query: command=%s response=%s parsed=%g arcsec/step",
               axisLabel, cmd, response, scale);
    return true;
}

bool LX200_OnStep_OpenAstroExplorer::parsePositiveFiniteDouble(const char *response, double &value) const
{
    if (response == nullptr)
        return false;
...
bool LX200_OnStep_OpenAstroExplorer::processPolarAdjustmentNumber(INDI::PropertyNumber &property, const char *axisLabel,
        double values[], char *names[], int n)
{
    property.update(values, names, n);

    const double signedDegrees = property[0].getValue();
    const bool valid = std::isfinite(signedDegrees) && signedDegrees >= -3.0 && signedDegrees <= 3.0;
    property.setState(valid ? IPS_OK : IPS_ALERT);
    property.apply();

    if (!valid)
        LOGF_ERROR("OAE %s correction is invalid: degrees=%g.", axisLabel, signedDegrees);
    else
        LOGF_DEBUG("OAE %s correction updated: degrees=%g.", axisLabel, signedDegrees);

    return valid;
}

bool LX200_OnStep_OpenAstroExplorer::formatPolarJogCommand(int axisNumber, int64_t rawSteps, char *cmd,
        size_t cmdLength) const
{
    const int written = snprintf(cmd, cmdLength, ":F%dr%lld#", axisNumber, static_cast<long long>(rawSteps));
    return written >= 0 && static_cast<size_t>(written) < cmdLength;
}

bool LX200_OnStep_OpenAstroExplorer::processPolarJog(INDI::PropertySwitch &commandProperty, const char *axisLabel,
        int axisNumber, INDI::PropertyNumber &offsetProperty, bool axisAvailable, double arcsecondsPerRawStep)
{
    const double signedDegrees = offsetProperty[0].getValue();
    if (!std::isfinite(signedDegrees) || signedDegrees < -3.0 || signedDegrees > 3.0)
    {
        LOGF_ERROR("OAE %s jog rejected: invalid correction degrees=%g.", axisLabel, signedDegrees);
        commandProperty.setState(IPS_ALERT);
        commandProperty.apply();
        return false;
    }

    if (!axisAvailable)
    {
        LOGF_ERROR("OAE %s jog rejected: F%d axis is unavailable.", axisLabel, axisNumber);
        commandProperty.setState(IPS_ALERT);
        commandProperty.apply();
        return false;
    }

    if (!std::isfinite(arcsecondsPerRawStep) || arcsecondsPerRawStep <= 0.0)
    {
        LOGF_ERROR("OAE %s jog rejected: invalid F%d scale=%g arcsec/step.",
                   axisLabel, axisNumber, arcsecondsPerRawStep);
        commandProperty.setState(IPS_ALERT);
        commandProperty.apply();
        return false;
    }

    const double signedArcseconds = signedDegrees * 3600.0;
    if (!std::isfinite(signedArcseconds))
    {
        LOGF_ERROR("OAE %s jog rejected: arcsecond conversion overflow for degrees=%g.",
                   axisLabel, signedDegrees);
        commandProperty.setState(IPS_ALERT);
        commandProperty.apply();
        return false;
    }

    const double rawStepsDouble = signedArcseconds / arcsecondsPerRawStep;
    if (!std::isfinite(rawStepsDouble) ||
            rawStepsDouble < static_cast<double>(std::numeric_limits<int64_t>::min()) ||
            rawStepsDouble > static_cast<double>(std::numeric_limits<int64_t>::max()))
    {
        LOGF_ERROR("OAE %s jog rejected: raw step conversion overflow for arcseconds=%g scale=%g.",
                   axisLabel, signedArcseconds, arcsecondsPerRawStep);
        commandProperty.setState(IPS_ALERT);
        commandProperty.apply();
        return false;
    }

    const int64_t rawSteps = std::llround(rawStepsDouble);
    char cmd[CMD_MAX_LEN] = {0};
    if (!formatPolarJogCommand(axisNumber, rawSteps, cmd, sizeof(cmd)))
    {
        LOGF_ERROR("OAE %s jog rejected: movement command formatting failed.", axisLabel);
        commandProperty.setState(IPS_ALERT);
        commandProperty.apply();
        return false;
    }

    LOGF_DEBUG("OAE %s jog: degrees=%g arcseconds=%g scale=%g arcsec/step raw_steps=%lld command=%s",
               axisLabel, signedDegrees, signedArcseconds, arcsecondsPerRawStep,
               static_cast<long long>(rawSteps), cmd);

    if (rawSteps == 0)
    {
        LOGF_INFO("OAE %s jog: no movement required after raw-step rounding.", axisLabel);
        commandProperty.setState(IPS_OK);
        commandProperty.apply();
        return true;
    }
    ...
    if (isSimulation())
    {
        LOGF_DEBUG("OAE simulation-mode command: %s", cmd);
        commandProperty.setState(IPS_OK);
        commandProperty.apply();
        return true;
    }
    ...
    {
        LOGF_ERROR("OAE %s jog command failed: %s", axisLabel, cmd);
        commandProperty.setState(IPS_ALERT);
        commandProperty.apply();
        return false;
    }
...
bool LX200_OnStep_OpenAstroExplorer::saveConfigItems(FILE *fp)
{
    LX200Generic::saveConfigItems(fp);
    WI::saveConfigItems(fp);
    OAEAltitudeJogNP.save(fp);
    OAEAzimuthJogNP.save(fp);
    return true;
}

void LX200_OnStep_OpenAstroExplorer::Init_Outputs()
{
    if (OSHasOutputs)
    {
...
bool LX200_OnStep_OpenAstroExplorer::sendScopeTime()
{
...
bool LX200_OnStep_OpenAstroExplorer::sendScopeLocation()
{
...
bool LX200_OnStep_OpenAstroExplorer::Goto(double ra, double dec)
{
    const struct timespec timeout = {0, 100000000L};
...
void LX200_OnStep_OpenAstroExplorer::SyncParkStatus(bool isparked)
{
...
void LX200_OnStep_OpenAstroExplorer::SetParked(bool isparked)
{
    PrintTrackState();
...
void LX200_OnStep_OpenAstroExplorer::PrintTrackState()
{
#ifdef DEBUG_TRACKSTATE
    switch(TrackState)
    {
...
bool LX200_OnStep_OpenAstroExplorer::setUTCOffset(double offset)
{
    bool result = true;
...
IPState LX200_OnStep_OpenAstroExplorer::ExecuteHomeAction(TelescopeHomeAction action)
{
...
bool LX200_OnStep_OpenAstroExplorer::Handshake()
{
    if (checkConnection())
    {
        return true;
...
void LX200_OnStep_OpenAstroExplorer::initSlewRates()
{
...
bool LX200_OnStep_OpenAstroExplorer::MoveNS(INDI_DIR_NS dir, TelescopeMotionCommand command)
{
...
bool LX200_OnStep_OpenAstroExplorer::MoveWE(INDI_DIR_WE dir, TelescopeMotionCommand command)
{
    if (command == MOTION_START)
    {
